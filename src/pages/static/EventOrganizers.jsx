      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {organizers.map((org, idx) => (
          <Slide key={org.email} direction={idx % 2 === 0 ? "left" : "right"}>
            <div className="group bg-base-100 rounded shadow-md hover:shadow-lg border-b-4 border-secondary transition-all duration-500 overflow-hidden flex flex-col items-center p-8 relative hover:-translate-y-1">
              {/* Decorative badge */}
              <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-gradient-to-r from-primary to-secondary p-2 rounded-full shadow-lg">
                <FiCheckCircle className="text-white text-2xl" />
              </div>
              <div className="overflow-hidden w-22 h-22 mb-4 rounded-full border-4 border-secondary/30 shadow-md flex items-center justify-center">
                <img
                  src={org.photo}
                  alt={org.name}
              ...
              <p className="text-accent text-center mb-4 text-sm">{org.bio}</p>
              <a
                href={`mailto:${org.email}`}
                className="btn btn-sm bg-gradient-to-r from-primary to-secondary text-white rounded shadow hover:scale-105 transition flex items-center gap-2"
              >
                <FiMail /> Contact
              </a>
            </div>
